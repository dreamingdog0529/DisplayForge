...
| ファイル名例 | `DisplayForge-0.1.0-win-x64-ja-JP.msi` / `…-en-US.msi`（インストーラー言語。アプリ UI は[31 言語](#対応言語)） |
...
GitHub Releases では現在 **en-US** と **ja-JP** の MSI を配布しています（インストーラーウィザードの表示言語のみの違い）。上記アプリ UI 言語はどちらのパッケージでも同じです。
